...
import org.krb.forum.controller.form.NewUserForm;
import org.krb.forum.controller.form.UserEditForm;
import org.krb.forum.controller.model.UserProfile;
import org.krb.forum.entity.User;
import org.krb.forum.entity.UserInfo;
import org.krb.forum.entity.type.Sex;
...
public class UserController {
    ...
    public String login(Model model, String error, String logout) {
    	if (error != null) {
            model.addAttribute("message", "user.login.error");
        	//model.addAttribute("error", "Your username or password is invalid.");
        }

        if (logout != null) {
        	model.addAttribute("message", "user.logout.successful");
    ...
    @RequestMapping(value = "/myprofile/edit/userinfo", method = RequestMethod.POST)
    public String editUserInfo(@Valid @ModelAttribute("userEditForm") UserEditForm userEditForm,
    		  Authentication authentication,
    		  BindingResult result,
    	...
    	try {
             userProfile = userProfileService.findOne(authentication.getName());
         } catch (NullPointerException e) {
             throw new UserNotFoundException();
        }
    	User editUser =  userProfile.getUser();
    	if (editUser.getInfo() == null) {
    		UserInfo info = new UserInfo();
    		info.setUser(editUser);
    		editUser.setInfo(info);
    	}
    	editUser.getInfo().setAboutMe(userEditForm.getBiography());
    	editUser.getInfo().setFooter(userEditForm.getFooter());
    	editUser.getInfo().setCity(userEditForm.getCity());
    	editUser.getInfo().setName(userEditForm.getName());
    	editUser.getInfo().setLastName(userEditForm.getLastName());
    	editUser.getInfo().setMale(userEditForm.getSex() == Sex.M ? true:false);
    	try {
    		Date date1 =new SimpleDateFormat("yyyy-MM-dd").parse(userEditForm.getBirthday());  
    		editUser.getInfo().setBirthday(date1);
    	} catch(Exception ex) {
    		
    	}
    	userService.save(editUser);
    
    	redirectModel.addFlashAttribute("message", "user.info.successfully.saved");
